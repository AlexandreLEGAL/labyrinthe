...
class LabyrintheGraphique(object):
    ...
    def __init__(self, labyrinthe, titre='Labyrinthe des IUT', size=(1000, 800), couleur=(209, 238, 238),
                 prefixeImage="./images"):
        """Method docstring."""
    ...
    def surfaceCarte(self, carte):
        ...
        dist = 10
        coord = [(dist, dist), (dist, self.deltah - (self.deltah // 4 + dist)),
                 (self.deltal - (self.deltal // 4 + dist), self.deltah - (self.deltah // 4 + dist)),
    ...
    def afficheCarteAJouer(self):
        ...
        """
        self.surface.blit(self.surfaceCarte(getCarteAJouer(self.labyrinthe)),
    ...
    def afficheJeu(self):
        ...
        self.afficheGrille()
        if not self.fini:
            self.afficheMessage(2, "C'est au joueur @img@ de jouer. Trésor à trouver @img@",
                                [self.surfacePion(numJoueurCourant(getListeJoueurs(self.labyrinthe))),
                                 self.surfaceTresor(tresorCourant(getListeJoueurs(self.labyrinthe)))])
        self.afficheScore(3)
